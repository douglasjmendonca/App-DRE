...
st.write('## Processo de Pagamentos')
...
with st.container(): # Processo de Pagamentos
    # Processo de Movimentação de Estoque
    def process_excel_pagamentos(uploaded_file):
    
        # Entrando com os dados de despesa contábil
        aba = pd.read_excel(uploaded_file, sheet_name=['MONTES CLAROS', 'CSC'])
        ...
        tratamento = DataFrame(columns = ['IDCC','IDCLVL','IDCONTA', 'EMPRESA', 'COD_FILIAL', 'NOME_FILIAL', 'COD_PRODUTO',
        ...
        # Excluindo os dados em que a coluna VALOR_REF for igual a zero
        tratamento = tratamento.loc[tratamento['VALOR_REF'] != 0]

        # Colocando os valores na coluna CIDADE
        tratamento['CIDADE'] = entrada['CIDADE']

        #Colocando os valores na coluna HISTORICO
        tratamento['HISTORICO'] = entrada['NUMERO'].astype(str) + ' PAGAMENTOS RW ' + entrada['NATUREZA'] + ' CODFORNE ' + entrada['CODFORNE'].astype(str) + ' FORNE ' + entrada['NOMFORNE']

        # Colocando valores na coluna COD_FORNECEDOR
        tratamento['COD_FORNECEDOR'] = entrada['CODFORNE']

        # Colocando os valores na coluna CONTA
        tratamento['CONTA'] = entrada['NATUREZA'].astype(str).str[7:]
        ...
        # Excluindo os dados em que a coluna CONTA for igual a EMPRESTIMO PRONAMPE
        tratamento = tratamento.loc[tratamento['CONTA'] != 'EMPRESTIMO PRONAMPE']

        # Colocando os valores na coluna DETALHAMENTO
        tratamento['DETALHAMENTO'] = entrada['NOMFORNE']

        # Colocando valores na coluna 'OBS'
        tratamento['OBS'] = entrada['OBSERV']

        # Criando o DataFrame de saída aproveitando algumas colunas do dados de tratamento
        saida = DataFrame(tratamento, columns = ['IDCC','IDCLVL','IDCONTA', 'EMPRESA', 'COD_FILIAL', 'NOME_FILIAL', 'COD_PRODUTO',
                                        'DESC_PRODUTO', 'QUANTIDADE', 'DATA', 'VALOR_REF', 'DOCUMENTO', 'HISTORICO', 
...
with st.container(): #Funções e botões para o processamento do arquivo
    ...
    nome_padraof = 'pagamentos_DRE_' + mes + ano + '.xlsx'
    ...
    with coluna2:
        ...
        if uploaded_file:
            ...
            processed_file = process_excel_pagamentos(uploaded_file)
